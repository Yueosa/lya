...
- [ ] 记忆索引的编号与排序（讨论中）。现在 `#N` 是位置编号，写一条记忆就全体重排，
      而历史里的 `已记住 #2「X」` 不会跟着改，于是同一个上下文里同一个编号指两条记忆。
      候选：编号改成稳定的库内 id、排序改 `id ASC` 追加式。连带决定 `pinned` 的去留——
      它现在没有任何写入路径
...
- [ ] **两套外壳的特色**。MTF 走宽松卡片（带偏移阴影、留白大），Minecraft 已做。
      边界不变：外壳可换，视图不可换（`src/shell/types.ts`）
- [ ] **跳到最新按钮**。右下角，四态：隐藏 / 正在跟随 / 输出完毕 / 百分比。
      同时解决「我翻到哪了」和「要不要跟随流式」
- [ ] 首页悬浮词的碰撞检测仍不准：`estimateBox()` 把 px 字号乘魔数当百分比用，量纲是错的；
      `occupiedBoxes()` 跳过正在淡出的标签，新词会生在它们身上。坐标跳动已修，这两条没修
- [x] SessionSettings 与 `usePrefs` 分区标注（「本机显示」vs「本会话」）（2026-08-05）
...
| 项 | 现状 | 什么时候做 |
|----|------|-----------|
| **记忆索引与前缀缓存冲突** | 记忆索引在 system prompt 里，而 system 是第一条消息——改一个字节就是整个请求全量 miss，不是尾部失效。且编号按位置算（`index_entries` 用 `enumerate`，排序 `pinned DESC, updated_at DESC`），**新增或更新任意一条记忆都会让后面编号全部顺延**，整段重写。`memory_read` 不改 `updated_at`，读不会触发。缓解方案：把索引挪到靠近末尾的独立消息，只失效尾部；代价是模型更容易把它当对话内容而不是设定 | 大重构时一并做 |
| 多模态图片输入 | `models.toml` 的 `vision` capability 已接进提示词（会下确定断言），但 `lya-llm` 侧**没有图片输入通道**——`ChatMessage.content` 是纯 String。所以声明了 vision 的模型实际也收不到图 | 补 `content` 多段结构时 |
| 媒体固有宽高进 `MediaMeta` | 视频靠 JS 读 `videoWidth/Height` 再写 `--local-media-ratio`，第一帧仍是 300×150；图片没有占位。原方案是服务端存下宽高，渲染时直接输出 `aspect-ratio` | 位置恢复还不够稳时 |
| 图片上传 | 无 | 依赖上面那条 |
| 分支命名 | 无表无 API；分支靠叶节点 id 区分（原先空建的 `branch_meta` 已随 schema 合并删掉） | WebUI 要显示分支名时 |
...
- [x] 数据库 schema 合并进 `lya-db/migrations/000_init.sql`：`Db::open` 即带全库 schema，
      业务 crate 不再建表；删 `branch_meta` 与种子记忆；测试统一 `open_test_db()`。
      老库用一次性脚本对齐过（删 `branch_meta` / `session_events` / 过期台账），脚本跑完已删（2026-08-05）
