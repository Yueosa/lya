...
已有 10 个，按权限分级（`tools::tests::builtin_tools_are_graded_by_permission`
钉住这张表）：

| 工具 | 权限 | 可见于 |
|------|------|--------|
| `file_read` / `dir_list` / `system_info` / `image_scan` / `web_search` / `web_fetch` | `-R-` | ask 起 |
| `file_write` / `file_edit` | `-R-W-` | edit 起 |
| `file_manage`（删/移/拷/信息/建目录） / `bash` | `-R-W-X-` | 仅 agent |

删除与移动划到执行级而不是写级：它们不可逆，和「改文件内容」不是一回事。

粒度上刻意比上一代粗——它光文件和目录就 16 个工具，我们 5 个覆盖同样的事。读和写
各自独立（用得最多，schema 要精确），低频的删/移/拷/信息/建目录合成一个
`file_manage`，省提示词预算；图片那三个同理合成 `image_scan`。
...
- **`web_fetch` 不能翻页**：只有 `max_chars` 截断，没有偏移量。长文档读不全时
  只能换更具体的页面。等真遇到了再加。
  ~~**`web_fetch` 不拦内网地址**~~：已做。HTTP 层落地后这条的触发条件就到了——
  lya 自己在本机监听，而它的跨站守卫**对服务端发起的请求完全无效**（守卫看
  `Origin`，`reqwest` 根本不发那个头），网页里的一句提示词注入就能读走明文密钥、
  全部记忆、全部对话。

  没有一刀切拦截，因为区分点不是「地址是不是内网」而是「URL 是谁选的」，
  而这种拿不准的场景正是确认流程的用途。分三档：**lya 自己硬拦**（没有正当用途，
  排查自己用 `bash` 里的 `curl`）、**其它内网走确认**（保住「看看我本机 3000 端口」，
  确认框里的地址由我们的代码从实际 URL 生成，注入伪装不了）、**公网照常**。

  两个容易漏的地方都堵了：判定在 **DNS 解析之后**做，否则 `localtest.me` 这类
  解析到 127.0.0.1 的域名直接绕过；**重定向落地地址再判一次**，否则外网页面
  302 到内网同样能绕。字面公网但解析到内网的一律拒绝而不是补确认——那时已经
  错过确认时机了。`tests/ssrf.rs` 起真实本地服务钉住这三条路径。

  自身端口用 `Arc<AtomicU16>` 回填：工具注册发生在监听之前，端口被占用时
  `candidate_ports` 还会往后退让。保持 0 也安全，那样本机地址全部走确认。
...
- **不做 `text_*`**（统计/diff/正则替换那六个）和 `http_request`：前者模型自己
  ...
  ~~**`bash` 需要自带命令解析与确认**~~：已做。命令按 `&&`、`|` 拆开逐段说明再走
  `HitlBlock::ToolConfirm`。**确认逻辑属于工具自己**，没做成所有工具通用的风险
  等级（见「明确不做」）；`web_fetch` 的内网确认复用的是同一条 `confirm_request`
  出口。落地时顺带解决了那个结构问题——工具现在也能通向 `AwaitHuman`，不再是
  只有 action 能挂起。
  ~~**工具内部没有超时**~~：已做。`bash` 有 `timeout_secs` 参数，跨轮的「停止本轮」
  仍由 `lya-agent` 的 `CancelToken` 覆盖。

### lya-memory

- **索引常驻优先于检索**：全部索引常驻 prompt，模型看得见所有条目、按编号读正文，
  比检索更准；`memory_search` 只是补充，用在索引被 `IndexBudget` 截断（超预算时
  只留最近更新的若干条）之后去捞没列出来的那些。
...
- **只有五个动作**：`memory_read` / `memory_write` / `memory_search` / `form` /
